          ...
          <div class="col">
              <div class="card">
                  ...
                  <div class="card-block">
                      <form class="form-material" method="POST" action="{{ route('usuarios.update',[$usuario->id,'jefedepartamento']) }}" enctype="multipart/form-data"> {{-- enctype="multipart/form-data" permite enviar archivos --}}
                        @csrf
                        @method('put')
                        <div class="row px-3">
                            <input type="hidden" name="rol_id" value="{{$usuario->rol_id}}" id="">
                            <input type="hidden" name="jefedepartamento_id" value="{{$usuario->jefedepartamento->id}}" id="">
                            ...
                            <div class="form-group col-md-4 form-default">
                                ...
                                <select class="form-control select2 select2-hidden-accessible selectpicker" aria-hidden="true" data-live-search="true" name="escuela_id" id="escuela" required>
                                    ...
                                    <option value="{{$escuela->id}}" {{($escuela->id==$usuario->jefedepartamento->escuela_id)?'selected':''}}>{{$escuela->descripcion}}</option>
                                    @empty
                                    <option value="">Sin Registros</option>
                                    @endforelse
                                </select>
