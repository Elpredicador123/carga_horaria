          ...
          <div class="col">
              <div class="card">
                  ...
                  <div class="card-block">
                      <form class="form-material" method="POST" action="{{ route('usuarios.store','docente') }}" enctype="multipart/form-data"> {{-- enctype="multipart/form-data" permite enviar archivos --}}
                        @csrf
                        <div class="row px-3">
                            ...
                            <div class="form-group col-md-2 form-default">
                                ...
                                <select class="form-control select2 select2-hidden-accessible selectpicker" disabled aria-hidden="true" data-live-search="true" name="rol_id" id="rol" required>
                                    ...
                                    @forelse ($rol as $r)
                                    <option value="{{$r->id}}" {{($r->descripcion=='docente')?'selected':''}}>{{$r->descripcion}}</option>
                                    @empty
                            ...
                            <div class="form-group col-md-3 form-default">
                                ...
                                <select class="form-control select2 select2-hidden-accessible selectpicker" aria-hidden="true" data-live-search="true" name="modalidad_id" id="modalidad" required>
                                    ...
                                    <option value="{{$modalidad->id}}">{{$modalidad->descripcion.' '.$modalidad->horas.' H'}}</option>
                                    @empty
                                    <option value="">Sin Registros</option>
                                    @endforelse
                                </select>
                        ...
                        <div class="text-center">
                          <button type="submit" onclick="return confirm('¿Guardar nuevo Usuario Docente?')" class="btn btn-primary" ><i class="fa fa-save"></i> Grabar</button>
